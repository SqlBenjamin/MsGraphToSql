...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'MsGraphSyncToSql_ReportExports')
BEGIN
    EXECUTE msdb.dbo.sp_delete_job @job_name = N'MsGraphSyncToSql_ReportExports', @delete_unused_schedule = 1;
    PRINT 'Job "MsGraphSyncToSql_ReportExports" Deleted.';
END;
GO

/********************************************************************************************
Object: dbo.MsGraphSyncToSql_ReportExports
Purpose: This job runs a PowerShell script to sync data from Graph to SQL.

History:
Date          Version    Author                   Notes:
03/09/2021    0.0        Benjamin Reynolds        Created.
...
----------------------------------------------------------------------------------------------
DECLARE  @JobName sysname = N'MsGraphSyncToSql_ReportExports';
DECLARE  @UrlsToSync nvarchar(max) = N'SqlFilter:JobName = N'''+@JobName+N''' AND Enabled = 1'
        ...
        ,@UseDbMailItems bit;
SELECT @PSCommand = N'cd "%SystemRoot%\system32\WindowsPowerShell\v1.0" && powershell.exe -Command "& '''+@ScriptLocation+N''' -SqlServerName '+@SqlServerName+N' -UrlsToSync '''+REPLACE(@UrlsToSync,N'''',N'''''')+N''' -GraphUser '''+@MSI_AppId+N''' -SqlUser ''me'' -GraphUserIsMSI -SqlLoggingTableJobName '''+@JobName+N''' -ProcessUrisWithoutPartType $false -ProcessSpecificURLs $false -ProcessDrillDownURLs $false -WriteBatchSize 400000 -BulkCopyTimeout 600 -BulkCopyBatchSize 50000;"
';
