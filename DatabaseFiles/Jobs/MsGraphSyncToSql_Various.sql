...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'MsGraphSyncToSql_Various')
BEGIN
    EXECUTE msdb.dbo.sp_delete_job @job_name = N'MsGraphSyncToSql_Various', @delete_unused_schedule = 1;
    PRINT 'Job "MsGraphSyncToSql_Various" Deleted.';
END;
GO

/********************************************************************************************
Object: dbo.MsGraphSyncToSql_Various
...
05/03/2021    0.0        Benjamin Reynolds        Updated to use SQL "UrlsToSync" logic. ?Add LogFullPath logic?
08/02/2021    0.0        Benjamin Reynolds        Added step to kick off the job that populates the history table.
...
----------------------------------------------------------------------------------------------
DECLARE  @JobName sysname = N'MsGraphSyncToSql_Various';
...
SELECT @StartJobCommand = N'IF EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE name = N''MsGraphSyncToSql_Populate_DCP_DeviceStatusOverviewHistory'')
EXECUTE msdb.dbo.sp_start_job @job_name = N''MsGraphSyncToSql_Populate_DCP_DeviceStatusOverviewHistory'';
';
...
EXECUTE @ReturnCode = msdb.dbo.sp_add_jobstep
        @job_id = @jobId
       ,@step_name = N'Kick off the DCP_DeviceStatusOverviewHistory job'
       ,@step_id = 2
       ,@cmdexec_success_code = 0
       ,@on_success_action = 3 -- Go to the next step
       ,@on_success_step_id = 0
