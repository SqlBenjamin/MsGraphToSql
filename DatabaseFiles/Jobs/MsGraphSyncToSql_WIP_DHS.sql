...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'MsGraphSyncToSql_WIP_DHS')
BEGIN
    EXECUTE msdb.dbo.sp_delete_job @job_name = N'MsGraphSyncToSql_WIP_DHS', @delete_unused_schedule = 1;
    PRINT 'Job "MsGraphSyncToSql_WIP_DHS" Deleted.';
END;
GO

/********************************************************************************************
Object: dbo.MsGraphSyncToSql_WIP_DHS
Purpose: This job runs a PowerShell script to sync data from Graph to SQL.

History:
Date          Version    Author                   Notes:
06/29/2020    0.0        Benjamin Reynolds        Created.
08/28/2020    0.0        Benjamin Reynolds        Added step to kick off DeviceRunStates job.
09/14/2020    0.0        Benjamin Reynolds        Added additional job to kick off: Populate Remediation History job.
...
08/02/2021    0.0        Benjamin Reynolds        Removed "IntuneSyncToSql_DeviceRunStates" from @StartJobCommand because the job has been archived.
...
----------------------------------------------------------------------------------------------
DECLARE  @JobName sysname = N'MsGraphSyncToSql_WIP_DHS';
DECLARE  @UrlsToSync nvarchar(max) = N'SqlFilter:JobName = N'''+@JobName+N''' AND Enabled = 1'
        ,@ReturnCode int = 0
        ,@JobCategory sysname = N'Data Collector'
        ,@TablesToSyncFile nvarchar(max) = N'G:\IntuneSync\TablesToSyncFiles\TablesToSync_WIP_DHS.txt'
...
END;
SELECT @StartJobCommand = N'IF EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE name = N''MsGraphSyncToSql_Populate_DHS_RemediationHistory'')
EXECUTE msdb.dbo.sp_start_job @job_name = N''MsGraphSyncToSql_Populate_DHS_RemediationHistory'';
';
...
EXECUTE @ReturnCode = msdb.dbo.sp_add_jobstep
        @job_id = @jobId
       ,@step_name = N'Kick off the DHS_RemediationHistory job'
       ,@step_id = 2
       ,@cmdexec_success_code = 0
       ,@on_success_action = 3 -- Go to the next step
       ,@on_success_step_id = 0
