...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'MsGraphSyncToSql_SpecificTable_adhoc')
BEGIN
    EXECUTE msdb.dbo.sp_delete_job @job_name = N'MsGraphSyncToSql_SpecificTable_adhoc', @delete_unused_schedule = 1;
    PRINT 'Job "MsGraphSyncToSql_SpecificTable_adhoc" Deleted.';
END;
GO

/********************************************************************************************
Object: dbo.MsGraphSyncToSql_SpecificTable_adhoc
...
05/04/2021    0.0        Benjamin Reynolds        Updated to use SQL "UrlsToSync" logic. ?Add LogFullPath logic?
...
DECLARE  @ReturnCode int = 0
        ,@JobName sysname = N'MsGraphSyncToSql_SpecificTable_adhoc'
        ,@JobCategory sysname = N'Data Collector'
        ,@UrlsToSync nvarchar(max) = N'SqlFilter:TableName = N''deviceManagementScripts'''
