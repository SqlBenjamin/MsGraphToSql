...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'MsGraphSyncToSql_Populate_DHS_RemediationHistory')
BEGIN
    EXECUTE msdb.dbo.sp_delete_job @job_name = N'MsGraphSyncToSql_Populate_DHS_RemediationHistory', @delete_unused_schedule = 1;
    PRINT 'Job "MsGraphSyncToSql_Populate_DHS_RemediationHistory" Deleted.';
END;
GO

/********************************************************************************************
Object: dbo.MsGraphSyncToSql_Populate_DHS_RemediationHistory
...
DECLARE  @ReturnCode int = 0
        ,@JobName sysname = N'MsGraphSyncToSql_Populate_DHS_RemediationHistory'
...
EXECUTE  @ReturnCode = msdb.dbo.sp_add_job 
        ...
        ,@notify_level_page = 0
        ,@delete_level = 0
        ,@description = N'This Populates/Deletes the data in DeviceHealthScriptsRemediationHistory'
...
EXECUTE @ReturnCode = msdb.dbo.sp_add_jobstep
        @job_id = @jobId
       ,@step_name = N'Populate the DeviceHealthScriptsRemediationHistory table with the new records'
       ,@step_id = 1
       ...
       ,@os_run_priority = 0
       ,@subsystem = N'TSQL'
       ,@command = N'EXECUTE dbo.usp_PopulateDHSRemediationHistoryTable;'
...
EXECUTE @ReturnCode = msdb.dbo.sp_add_jobstep
        @job_id = @jobId
       ,@step_name = N'Delete older records from DeviceHealthScriptsRemediationHistory'
       ,@step_id = 2
       ,@cmdexec_success_code = 0
       ...
       ,@os_run_priority = 0
       ,@subsystem = N'TSQL'
       ,@command = N'EXECUTE dbo.usp_DeleteDHSRemediationHistoryTable @DaysToKeep = 183;'
       ,@database_name = N'Intune'
       ,@flags = 0;
IF (@@ERROR != 0 OR @ReturnCode != 0)
GOTO QuitWithRollback;
